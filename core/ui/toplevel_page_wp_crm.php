...
<div class="wp_crm_ajax_result"></div>
    <?php screen_icon(); ?>
    <h2><?php _e('CRM - All People', 'wp_crm'); ?> <?php if(current_user_can('WP-CRM: View Profiles') && (current_user_can('create_users') || current_user_can('add_users'))) { ?><a href="<?php echo admin_url('admin.php?page=wp_crm_add_new'); ?>" class="button add-new-h2"><?php _e('Add New', 'wp_crm'); ?></a><a href="/sync-shipments.php" class="button add-new-h2"><?php _e('Sync Shipments', 'wp_crm'); ?></a><?php } ?></h2>
