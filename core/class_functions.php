...
class WP_CRM_F {
      ...
      foreach ($search_vars as $primary_key => $key_terms) {
        ...
        if ($primary_key == 'search_string') {
          ...
          if ($tofind) {
            ...
            /* Now go through the boxes table */
            $where .= " OR u.ID IN (SELECT user_id FROM $tablename WHERE LOWER(order_id) LIKE '%$tofind%' OR LOWER(date_requested) LIKE '%$tofind%' OR LOWER(date_completed) LIKE '%$tofind%' OR LOWER(ship_status) LIKE '%$tofind%' OR LOWER(tracking_id) LIKE '%$tofind%')";
            $where .= ")";
          }
          continue;
        }
  ...
  static function maybe_install_tables() {
    ...
    if ( empty($wpdb->boxes) ) {
      $wpdb->boxes = $wpdb->base_prefix . 'boxes';
    }
